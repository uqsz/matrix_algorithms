...
def create_tree(t_min, t_max, s_min, s_max, r, eps):
    U, D, V = randomized_svd(A[t_min:t_max, s_min:s_max], n_components=r+1, random_state=0)
    if len(D) <= r or D[r] < eps:
        ...
        v.append(create_tree(t_min, t_new_max, s_new_max + 1, s_max, r, eps))
        v.append(create_tree(t_new_max + 1, t_max, s_min, s_new_max, r, eps))
        v.append(create_tree(t_new_max + 1, t_max, s_new_max + 1, s_max, r, eps))
        # TODO: połączyć macierze od dzieci w jedną macierz
        # v.children.sort(key=lambda x : (x.size[0], x.size[2]))
        # v.U = np.vstack([np.hstack([v.children[0].U, v.children[1].U]), np.hstack([v.children[2].U, v.children[3].U])])
        # v.V = np.vstack([np.hstack([v.children[0].V, v.children[1].V]), np.hstack([v.children[2].V, v.children[3].V])])
    return v
...
v = create_tree(0, n, 0, n, 1, D[n - 1]) # r to w pseudokodzie parametr b, eps to parametr sigma
B = v.U @ v.V
# print(A)
# # print(B)
# # print(U @ np.diag(D) @ V)
