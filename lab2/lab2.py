...
def test(n):  # test function
    ...
    plt.clf()
    plt.plot(tab_k, inverse_time, 'o-', label='rekurencja')
    plt.plot(tab_k, inverse_gauss_time, 'o-', label='iteracja')
    plt.title("Czas obliczania odwrotności od rozmiaru macierzy (2^k x 2^k)")
    plt.xlabel("k")
    plt.ylabel("Czas [s]")
    plt.legend()
    plt.savefig("lab2/graphs/inverse_time.png")

    plt.clf()
    plt.plot(tab_k, inverse_all, 'o-', label="rekurencja")
    plt.plot(tab_k, inverse_gauss_all, 'o-', label="iteracja")
    plt.legend()
    plt.title("Liczba operacji od rozmiaru macierzy (2^k x 2^k)")
    plt.xlabel("k")
    plt.ylabel("Liczba operacji")
    plt.semilogy()
    plt.savefig("lab2/graphs/inverse_operations.png")

    plt.clf()
    plt.plot(tab_k, inverse_gauss_a, 'o-', label="iteracja: dodawanie")
    plt.plot(tab_k, inverse_gauss_m, 'o-', label="iteracja: mnożenie")
    plt.plot(tab_k, inverse_a, 'o-', label="rekurencja: dodawanie")
    plt.plot(tab_k, inverse_m, 'o-', label="rekurencja: mnożenie")
    plt.legend()
    plt.title("Liczba operacji od rozmiaru macierzy (2^k x 2^k)")
    plt.xlabel("k")
    plt.ylabel("Liczba operacji")
    plt.semilogy()
    plt.savefig("lab2/graphs/inverse_operations1.png")
    ...
    plt.xlabel("k")
    plt.ylabel("Czas [s]")
    plt.savefig("lab2/graphs/LU_time.png")
    ...
    plt.ylabel("Liczba operacji")
    plt.semilogy()
    plt.savefig("lab2/graphs/LU_operations.png")
    ...
    plt.xlabel("k")
    plt.ylabel("Czas [s]")
    plt.savefig("lab2/graphs/determinant_time.png")
    ...
    plt.ylabel("Liczba operacji")
    plt.semilogy()
    plt.savefig("lab2/graphs/determinant_operations.png")
...
if __name__ == "__main__":
    ...
    cnt_a = 0
    cnt_m = 0
    A = [[0.3996362682016685, 0.3775324097440138, 0.2867355288648659, 0.9335904617261223],
         [0.8442372135947669, 0.6695498023698692,
             0.4219842325585101, 0.4341797318639589],
         [0.026568629132153628, 0.6848164865354961,
             0.006902246086230951, 0.49542134706786245],
         [0.5269780713781076, 0.8534918403914511, 0.5867139799665525, 0.08457429827957923]]
    show_matrix(A)
    show_matrix(inverse(A))
    show_matrix(gauss_inverse(A))
    show_matrix(LU(A))
    print(determinant(A))
...
# % Definicja macierzy 4x4
# A = [ 0.3996362682016685, 0.3775324097440138, 0.2867355288648659, 0.9335904617261223;
#         0.8442372135947669, 0.6695498023698692, 0.4219842325585101, 0.4341797318639589;
#         0.026568629132153628, 0.6848164865354961, 0.006902246086230951, 0.49542134706786245;
#         0.5269780713781076, 0.8534918403914511, 0.5867139799665525, 0.08457429827957923
# ];

# disp(inv(A));

# % Dokonaj faktoryzacji LU
# [L, U, P] = lu(A);

# % Wyświetl macierze L, U
# disp(L);
